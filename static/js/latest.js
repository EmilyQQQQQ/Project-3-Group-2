function init(){
  ...
  fetchDataButton.addEventListener("click", () => {
      ...
      // Template for the USGS API URL
      const usgsApiTemplate = "https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/{MAGNITUDE}_{TIME_PERIOD}.geojson";
  
      // Replace placeholders with selected values
      const usgsApiUrl = usgsApiTemplate
  ...
  function BarChart(data) {
    ...
    let locations = data.map(earthquake => earthquake.properties.place);
    let magnitudes = data.map(earthquake => earthquake.properties.mag);
    let depths = data.map(earthquake => earthquake.geometry.coordinates[2]);
  
    let trace1 = {
      y: locations.slice(0, 10).reverse(),
      x: magnitudes.slice(0, 10).reverse(),
      text: depths.slice(0, 10).reverse(),
      type: 'bar',
      orientation: 'h'
    };
